...
# Tourism Project/.env, i.e. three levels up from this file
# (database/config.py -> backend/ -> App/ -> Tourism Project/) — same
# root predict.py already loads SUPABASE_DB_URL from, so both credential
# sets now come from the one place regardless of launch directory.
_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent
load_dotenv(_PROJECT_ROOT / ".env")
# Back-compat: also pick up App/backend/database/.env if someone followed
# the older per-module .env.example instructions instead.
